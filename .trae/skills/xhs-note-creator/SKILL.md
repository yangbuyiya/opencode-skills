...
#### 脚本选择要求
- 主动和用户说使用 `render_xhs_v2`脚本还是 `render_xhs`脚本，根据用户需求选择。
- 两个脚本的区别告诉用户：
  - `render_xhs_v2`：支持更丰富的图片卡片渲染功能，如自定义 Emoji、标题、副标题等。
  - `render_xhs`： simpler version，仅支持基本的图片卡片渲染功能。
- 默认使用 `render_xhs_v2`脚本。
